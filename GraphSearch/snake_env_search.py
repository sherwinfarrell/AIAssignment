...
class Snake(gym.Env):
    ...
    def get_snake_pos(self):
        return self.snake.xcor(), self.snake.ycor()
    ...
    def node_body_check_apple(self, node):
        if len(node) > 0:
            for body in node[:]:
                if math.sqrt((body[0]-self.apple.xcor())**2 + (body[1]-self.apple.ycor())**2) < 20:
                    return True
    ...
    def node_wall_check(self, x, y):
        if x > 200 or x < -200 or y > 200 or y < -200:
            return True
        
    def node_body_check_snake(self, node):
        if len(node) > 1:
            for body in node[1:]:
                if math.sqrt((body[0]-node[0][0])**2 + (body[1]-node[0][1])**2) < 20:
                    return True
    ...
    def getSuccessors(self, node):
        ...
        for action in [0, 1, 2, 3]:
            ...
            if len(futureNode) > 0:
                for index in range(len(futureNode)-1, 0, -1):
                    x = futureNode[index-1][0]
                    y = futureNode[index-1][1]
                    futureNode[index][0] = x
                    futureNode[index][1] = y

            futureNode[0][0] = nextx
            futureNode[0][1] = nexty
            if not self.node_wall_check(nextx, nexty) and not self.node_body_check_snake(futureNode):
                cost = 1
                successors.append( ( futureNode, action, cost) )
        return successors
