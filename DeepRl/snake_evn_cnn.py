...
class Snake(gym.Env):
    ...
    def reset(self):
        ...
        np_img[ snake_y + 225 -1, snake_x + 225 -1] = 256
        np_img[ snake_y + 225 -2, snake_x + 225 -1] = 256
        np_img[ snake_y + 225 , snake_x + 225 -1] = 256
        np_img[ snake_y + 225 -1, snake_x + 225 -2] = 256
        np_img[ snake_y + 225 -1, snake_x + 225] = 256


        np_img[ apple_y + 225 -1,  apple_x + 225 -1] = 256
        np_img[ apple_y + 225 -1,  apple_x + 225 -2] = 256
        np_img[ apple_y + 225 -1 , apple_x + 225   ] = 256
        np_img[ apple_y + 225 -2,  apple_x + 225 -1] = 256
        np_img[ apple_y + 225,     apple_x + 225 -1] = 256


        np_img[apple_x + 225 -1, apple_y + 225 - 1 ] = 256


        for a in self.snake_body:
            np_img[ a.ycor() + 225 -1 , a.xcor() + 225 -1 ] = 256
            np_img[ a.ycor() + 225 -1 , a.xcor() + 225 -2 ] = 256
            np_img[ a.ycor() + 225 -1 , a.xcor() + 225    ] = 256
            np_img[ a.ycor() + 225 -2 , a.xcor() + 225 -1 ] = 256
            np_img[ a.ycor() + 225    , a.xcor() + 225 -1 ] = 256
        ...
        status = cv2.imwrite('before_state.png',np_img)

        # cv2.imshow('image',np_img)
        # cv2.waitKey(0)

        np_img = cv2.resize(np_img, dsize=dim, interpolation=cv2.INTER_AREA)

        status = cv2.imwrite('after_state.png',np_img)

        new_img = np.zeros((45,45))
        new_img[ math.floor((-snake_y + 225)/10) , math.floor((snake_x + 225 -1)/10)] = 256
        new_img[ math.floor((-apple_y + 225)/10) , math.floor((apple_x + 225 -1)/10)] = 256
        # new_img = cv2.flip(np_img, 0)
        for a in self.snake_body:
            new_img[ math.floor((-a.ycor() + 225)/10) , math.floor((a.xcor() + 225)/10) ] = 256


        status = cv2.imwrite('resize.png',new_img)
        # cv2.imshow('image',np_img)
        # cv2.waitKey(0)
        print(np_img.shape)
        state = new_img
    ...
    def step(self, action):
        ...
        new_img[ math.floor((-apple_y + 225)/10) , math.floor((apple_x + 225 -1)/10)] = 256
        for a in self.snake_body:
            new_img[ math.floor((-a.ycor() + 225)/10) , math.floor((a.xcor() + 225-1)/10) ] = 256
        # new_img = cv2.flip(np_img, 0)
