    ...
    for i, elem in enumerate(elems):
        for j, layer in enumerate(elem):
            for k in range(len(layer)):
                if random.random() < params["mutate_chance"]:
                    elems[i][j][k] *= mutation_factor()

    new_network = DQN(env, params, elems)
    return new_network
...
def breed(a, b):
    ...
    #print(weights)
    new_network = DQN(env, params, weights)
